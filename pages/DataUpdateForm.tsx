...
import { Loader2, User, Home, Book, CreditCard, Shield, Heart, UploadCloud, FileCheck2, X, Plus, Trash2 } from 'lucide-react';
...
const FileUpload: React.FC<{ label: string; fileUrl?: string; onFileChange: (file: File | null) => void }> = ({ label, fileUrl, onFileChange }) => {
    const [fileName, setFileName] = useState<string | null>(null);
    const inputRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        if (fileUrl) setFileName(fileUrl.split('/').pop()?.split('?')[0] || 'File tersimpan');
    }, [fileUrl]);
            ...
            >
                ...
                {fileName ? (
                    ...
                    <>
                        ...
                        <p className="ml-2 text-sm text-gray-500">Unggah file</p>
                    </>
                )}
            </div>
        </div>
...
const DataUpdateForm: React.FC<DataUpdateFormProps> = ({ employee, onCreateSubmission }) => {
    ...
    useEffect(() => {
        // Deep copy employee data to avoid mutating props
        ...
        try {
            ...
            for (const key in files) {
                ...
                if (file) {
                    ...
                    // Update the correct field in updatedData
                    if(key === 'profilePhoto') {
            ...
            // Form is now locked on EmployeePortal based on pendingSubmission.
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <FormSection icon={<User className="w-5 h-5"/>} title="Data Pribadi" description="Informasi dasar dan identitas diri Anda.">
                ...
                <FormField label="Usia" name="age" value={calculateAge(formData.birthDate)} onChange={() => {}} disabled />
                ...
                <FormField label="Status Pernikahan" name="maritalStatus" value={formData.maritalStatus} onChange={handleChange} type="select" options={[{value:'Belum Menikah', label:'Belum Menikah'}, {value:'Menikah', label:'Menikah'}, {value:'Cerai Hidup', label:'Cerai Hidup'}, {value:'Cerai Mati', label:'Cerai Mati'}]}/>
                 ...
                 <FileUpload label="Foto Profil" fileUrl={formData.profilePhotoUrl} onFileChange={(file) => handleFileChange('profilePhoto', file)} />
            </FormSection>

            {/* Alamat */}
            <FormSection icon={<Home className="w-5 h-5"/>} title="Informasi Alamat" description="Alamat sesuai KTP dan alamat domisili saat ini.">
                <h4 className="md:col-span-2 font-bold text-slate-700 text-base mt-2">Alamat KTP</h4>
                <FormField className="md:col-span-2" label="Alamat Lengkap (Jalan, No. Rumah)" name="addressKtp.address" value={formData.addressKtp?.address} onChange={handleChange} />
                ...
                <div className="md:col-span-2 mt-4 pt-4 border-t border-slate-100">
                    <h4 className="font-bold text-slate-700 text-base">Alamat Domisili</h4>
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <label htmlFor="isSameAsKtp" className="text-sm font-medium text-gray-700">Sama dengan alamat KTP</label>
                    </div>
                </div>
                {!formData.addressDomicile?.isSameAsKtp && (
                    <>
                        <FormField className="md:col-span-2" label="Alamat Lengkap Domisili" name="addressDomicile.address" value={formData.addressDomicile?.address} onChange={handleChange} />
                        <FormField label="RT Domisili" name="addressDomicile.rt" value={formData.addressDomicile?.rt} onChange={handleChange} />
                        <FormField label="RW Domisili" name="addressDomicile.rw" value={formData.addressDomicile?.rw} onChange={handleChange} />
                        <FormField label="Kelurahan / Desa Domisili" name="addressDomicile.village" value={formData.addressDomicile?.village} onChange={handleChange} />
                        <FormField label="Kecamatan Domisili" name="addressDomicile.district" value={formData.addressDomicile?.district} onChange={handleChange} />
                        <FormField label="Kota / Kabupaten Domisili" name="addressDomicile.city" value={formData.addressDomicile?.city} onChange={handleChange} />
                        <FormField label="Provinsi Domisili" name="addressDomicile.province" value={formData.addressDomicile?.province} onChange={handleChange} />
                        <FormField label="Kode Pos Domisili" name="addressDomicile.postalCode" value={formData.addressDomicile?.postalCode} onChange={handleChange} />
                    </>
                )}
            </FormSection>
            
            {/* Keluarga & Kontak Darurat */}
            <FormSection icon={<Heart className="w-5 h-5"/>} title="Keluarga & Kontak Darurat" description="Informasi keluarga dan kontak yang bisa dihubungi saat darurat.">
                <FormField label="Nama Ibu Kandung" name="familyData.motherName" value={formData.familyData?.motherName} onChange={handleChange} />
                <FormField label="Tanggal Lahir Ibu" name="familyData.motherBirthDate" value={formData.familyData?.motherBirthDate} onChange={handleChange} type="date" />
                <FormField label="Nama Ayah Kandung" name="familyData.fatherName" value={formData.familyData?.fatherName} onChange={handleChange} />
                <FormField label="Tanggal Lahir Ayah" name="familyData.fatherBirthDate" value={formData.familyData?.fatherBirthDate} onChange={handleChange} type="date" />
                ...
                <FormField label="Hubungan Kontak Darurat" name="emergencyContact.relationship" value={formData.emergencyContact?.relationship} onChange={handleChange} />
                <FormField label="No. HP Kontak Darurat" name="emergencyContact.phone" value={formData.emergencyContact?.phone} onChange={handleChange} />
                
                <div className="md:col-span-2 mt-4">
                  <h4 className="font-semibold text-slate-700 mb-2">Data Anak</h4>
                  {(formData.familyData?.childrenData || []).map((child, index) => (
                      <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-2 mb-2 p-2 bg-slate-50 rounded-lg">
                  ...
                  <button type="button" onClick={addChild} className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-800 mt-2">
             ...
             {/* Pendidikan */}
            <FormSection icon={<Book className="w-5 h-5"/>} title="Riwayat Pendidikan" description="Detail pendidikan terakhir yang Anda tempuh.">
                <FormField label="Pendidikan Terakhir" name="lastEducation" value={formData.lastEducation} onChange={handleChange} type="select" options={EDUCATION_LEVELS.map(l => ({value: l, label: l}))} />
            ...
            {/* Finansial & Pajak */}
            <FormSection icon={<CreditCard className="w-5 h-5"/>} title="Finansial & Pajak" description="Informasi untuk keperluan payroll dan administrasi pajak.">
                 <FormField label="Nomor NPWP" name="npwp" value={formData.npwp} onChange={handleChange} />
                 <FileUpload label="Foto NPWP" fileUrl={formData.documents?.photoNpwpUrl} onFileChange={(file) => handleFileChange('photoNpwp', file)} />
                 ...
                 <FormField className="md:col-span-2" label="Nama Pemilik Rekening" name="bankAccount.holderName" value={formData.bankAccount?.holderName} onChange={handleChange} />
            </FormSection>
            
            {/* Jaminan Sosial & Dokumen */}
            <FormSection icon={<Shield className="w-5 h-5"/>} title="Dokumen Pribadi" description="Unggah dokumen pendukung pribadi Anda.">
                 <FileUpload label="Foto KTP" fileUrl={formData.documents?.photoKtpUrl} onFileChange={(file) => handleFileChange('photoKtp', file)} />
                 <FileUpload label="Foto Kartu Keluarga (KK)" fileUrl={formData.documents?.photoKkUrl} onFileChange={(file) => handleFileChange('photoKk', file)} />
            </FormSection>

            <div className="pt-6 border-t border-slate-200 flex justify-end">
                 <button type="submit" disabled={isLoading} className="flex items-center justify-center gap-2 w-full md:w-auto bg-blue-600 text-white font-bold px-8 py-3 rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-500/20 active:scale-95 disabled:bg-blue-300">
                    {isLoading && <Loader2 className="w-5 h-5 animate-spin" />}
                    {isLoading ? 'Mengirim...' : 'Kirim Perubahan untuk Review'}
                </button>
            </div>
        </form>
    );
